describe('Google', () => {
    it('Search for university webpage and check university logo is present', () => {
        ...
        cy.get('input[title="Search"]').type("Canterbury Chirt Chuch{enter}")
        cy.contains('canterbury.ac.uk').scrollIntoView().click();
        cy.get('button').contains("#onetrust-accept-btn-handler").contains("Accept All").scrollIntoView().click();

        //cy.visit("https://canterbury.ac.uk")
